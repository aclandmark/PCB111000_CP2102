...
{MCUSR = 0;																	//Clear reset flags
	...
	asm("jmp 0x0000");}														//and operate the user application
	setRunBL_bit;}															//User switch pressed	Run bootloader

if((WD_RF_bit_set)&& (RunBL_bit_clear))asm("jmp 0x0000");					//User app runs following a WDTout
...
MCUCR = (1<<IVSEL);
	...
	while(1){
		while(1){															//User prompt
			do{sendString("p/r/t/T  ");}while((isCharavailable(100) == 0));
			keypress = receiveChar();
			if (keypress == 'p') break;
			if(keypress == 'r') {clear_RunBL_bit;
